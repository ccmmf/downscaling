...
This session downscales output from a PEcAn ensemble run. In this workflow we have
used the  `magic-ensemble` CLI to run SIPNET at a representative sample of fields
from the CADWR Land Use dataset.

The downscale step takes the ensemble output, fits a 
Random Forest model to the subset of fields, and then uses that model to predict
SIPNET outputs for every field in the CADWR Land Use dataset. 

The result is field-level
carbon, GHG, estimates for every crop field. Once the downscaling is complete, it 
then aggregates the output to zonal statistics, in this case, county and state SOC and GHG inventories.
...
state. The full analysis may take five or more minutes. 
...
second command copies the bundle's configuration to `config.yaml`. Review that
