...
[Environment setup](CARB-downscaling-setup.md) have been completed. In a new terminal
session, follow its
[startup instructions](CARB-downscaling-setup.md#every-time-you-start-a-new-terminal-session).
...
| Field | `area_ha` | Field area, also used to convert predicted density to a field total |
| PFT | `pft_annual_crop`, `pft_woody_perennial_crop` | Indicators of the assigned PFT |
...
The supplied configuration uses these values:
...
The maps show each predicted field's mean soil carbon density across the 10
ensemble members.
