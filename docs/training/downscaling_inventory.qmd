...
then predicts carbon stocks and greenhouse gas fluxes for every crop field in the target dataset. 
It aggregates the field-level predictions into county and statewide inventories.

The demo covers four Sacramento–San Joaquin Delta counties: Yolo, Solano, Sacramento, and Contra Costa. 
It trains on 99 design points distributed across California and predicts outcomes for fields within those four counties. 
The full analysis may take five minutes or more, depending on available computing resources.

::: {.callout-note}
## Scope

The demo dataset includes data layers for four counties. A statewide run includes all 57 and uses
...
The download command reads the S3 profile from `example_user_config.yaml`. The
second command copies the downloaded configuration to `config.yaml`. Review that
file below and use it for all remaining workflow commands.
...
- `mode` -- keep `production`. The other modes subsample: `demo` uses one ensemble
  member and one year, `dev` to ten sites. The demo dataset is already small because of its
  data scope and does not need further subsampling.
...
- `pecan_output_dir`, `data_layers_dir` -- paths to the downloaded model output and data layers
- `n_cores`
...
Use these two checks to assess the predictions before presenting the maps.
...
Out-of-bag R^2^ measures the forest's predictive performance:
...
Runtime depends on the available hardware. Use the table above to plan memory and
disk space. During downscaling, predicting across more target fields or ensemble
members requires more resources. Reducing the number of training sites alone does
not reduce the number of field-level predictions.

Note `prepare` copies `pecan_output_dir` into `run_dir`, so a statewide run needs room
for two copies of the ensemble output.

## Scenarios

Practice-change scenarios use the same four commands. The difference is that
`management_scenarios` lists several scenarios, the ensemble output contains one
`output_<scenario>/` directory per scenario, and the analysis adds scenario-minus-baseline
