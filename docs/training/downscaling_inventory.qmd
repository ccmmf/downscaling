...
A few variables specific to my own environment. These will take different values on
your system.

```bash
export DEMO_DIR=/path/to/your/workdir
export CONDA_DIR=~/.conda/envs
```
...
bash setup-pecan-env.sh 1.18 ${CONDA_DIR}/pecan-all-1.18
```
...
```bash
export AWS_PROFILE=magic
conda activate ${CONDA_DIR}/pecan-all-1.18
```

## Clone the downscaling repo (once)
...
The CLI lives at the repo root and finds its manifest beside itself, so run everything
from here.
...
cp demo-data/demo_config.yaml config.yaml
```
...
| Step | Demo (4 counties) | Statewide |
|---|---|---|
| prepare | 3.5 min | depends on disk |
| extract | 3.5 min | 13 min, 149 GB |
| downscale | 1.5 min | 53 min, 386 GB |
| analyze | 1 min | 7 min, 20 GB |
