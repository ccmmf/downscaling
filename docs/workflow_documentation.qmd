...
```sh
Rscript scripts/01_cluster_and_select_design_points.R
```
...
```sh
Rscript scripts/02_extract_sipnet_output.R
```
...
```sh
Rscript scripts/03_downscale_and_aggregate.R
```
...
**Inputs:**
- `out/efi_ens_long.csv`: SIPNET outputs
...
Each ensemble member represents a plausible realization given parameter uncertainty. We maintain this ensemble structure throughout the workflow to properly propagate uncertainty.
